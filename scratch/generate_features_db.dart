...
Map<String, dynamic> extractFeatures(img.Image decoded, String name) {
  ...
  if (objectPixelCount == 0) return {'organismName': name, 'error': 'Empty sprite'};
  ...
  finalHueBins['hWhite'] = 0; finalHueBins['hBlack'] = 0; finalHueBins['hGrey'] = 0;
  ...
  for (int y = 0; y < resized.height; y++) {
    for (int x = 0; x < resized.width; x++) {
      ...
      if (v < 0.15) finalHueBins['hBlack'] = (finalHueBins['hBlack'] ?? 0) + 1;
      else if (s < 0.15) {
        if (v > 0.8) finalHueBins['hWhite'] = (finalHueBins['hWhite'] ?? 0) + 1;
        else finalHueBins['hGrey'] = (finalHueBins['hGrey'] ?? 0) + 1;
      } else {
        final bin = (h / 10).floor().clamp(0, 35);
        finalHueBins['h${bin * 10}'] = (finalHueBins['h${bin * 10}'] ?? 0) + 1;
      }
      totalSaturation += s; totalBrightness += v;
  ...
  final dominantColors = sortedColors.take(8).map((e) {
    final q = e.key;
    final r = ((q >> 8) & 0xF) * 17, g = ((q >> 4) & 0xF) * 17, b = (q & 0xF) * 17;
  ...
  for (int gy = 0; gy < 3; gy++) {
    for (int gx = 0; gx < 3; gx++) {
      final sX = minX + (gx * (maxX - minX) ~/ 3), eX = minX + ((gx + 1) * (maxX - minX) ~/ 3);
      final sY = minY + (gy * (maxY - minY) ~/ 3), eY = minY + ((gy + 1) * (maxY - minY) ~/ 3);
      int gridPix = 0; final Map<int, int> bins = {};
      for (int y = sY; y < eY; y++) {
        for (int x = sX; x < eX; x++) {
          ...
          if (idx >= 0 && idx < mask.length && mask[idx]) {
            ...
            bins[b] = (bins[b] ?? 0) + 1; gridPix++;
          }
        }
      }
      bins.forEach((b, c) => spatialHueBins['g$gx${gy}_h${b * 10}'] = gridPix > 0 ? c / gridPix : 0);
    }
  }

  final cMinX = minX + ((maxX - minX) * 0.25).toInt(), cMaxX = maxX - ((maxX - minX) * 0.25).toInt();
  final cMinY = minY + ((maxY - minY) * 0.25).toInt(), cMaxY = maxY - ((maxY - minY) * 0.25).toInt();
  int corePix = 0;
  for (int y = cMinY; y <= cMaxY; y++) {
    for (int x = cMinX; x <= cMaxX; x++) if (mask[y * resized.width + x]) corePix++;
  }
  final coreSolidity = corePix / max(1, (cMaxX - cMinX + 1) * (cMaxY - cMinY + 1));
  ...
  final verticalBias = (top40 + bottom60) > 0 ? bottom60 / (top40 + bottom60) : 0.5;

  int maxRW = 0, maxRY = minY, minRW = maxX - minX + 1; List<int> rowWs = [];
  for (int y = minY; y <= maxY; y++) {
    int rw = 0; for (int x = minX; x <= maxX; x++) if (mask[y * resized.width + x]) rw++;
    if (rw > 0) { if (rw > maxRW) { maxRW = rw; maxRY = y; } if (rw < minRW) minRW = rw; rowWs.add(rw); }
  }
  ...
  for (int x = minX; x <= maxX; x++) {
    int ch = 0; for (int y = minY; y <= maxY; y++) if (mask[y * resized.width + x]) ch++;
    if (ch > maxCH) { maxCH = ch; maxCX = x; }
  }
  final maxHeightColBias = (maxX > minX) ? (maxCX - minX) / (maxX - minX) : 0.5;

  final bcMinX = minX + ((maxX - minX) * 0.35).toInt(), bcMaxX = maxX - ((maxX - minX) * 0.35).toInt();
  final bcMinY = maxY - ((maxY - minY) * 0.3).toInt();
  int bcPix = 0;
  for (int y = bcMinY; y <= maxY; y++) {
    for (int x = bcMinX; x <= bcMaxX; x++) if (mask[y * resized.width + x]) bcPix++;
  }
  final bottomCenterDensity = bcPix / max(1, (bcMaxX - bcMinX + 1) * (maxY - bcMinY + 1));

  final sym = _calculateSymmetry(resized, mask, minX, maxX, minY, maxY);
  final hSym = sym.$1, vSym = sym.$2, midX = minX + (maxX - minX) ~/ 2, midY = minY + (maxY - minY) ~/ 2;
  int qM = 0;
  ...
  for (int y = 0; y < resized.height; y++) {
    for (int x = 0; x < resized.width; x++) {
      if (!mask[y * resized.width + x]) continue;
      bool isE = (x == 0 || x == resized.width - 1 || y == 0 || y == resized.height - 1 ||
  ...
  int tXPos = 0, tYPos = 0;
  for (int y = minY; y <= maxY; y++) {
    for (int x = minX; x <= maxX; x++) if (mask[y * resized.width + x]) { tXPos += x; tYPos += y; }
  }
  ...
  double sumYDist = 0;
  for (int y = minY; y <= maxY; y++) {
    for (int x = minX; x <= maxX; x++) if (mask[y * resized.width + x]) sumYDist += pow(y - centroidY, 2);
  }
  final verticalMassDistribution = (maxY > minY) ? sqrt(sumYDist / objectPixelCount) / (maxY - minY) : 0.0;

  int clustered = 0;
  for (int y = minY + 1; y <= maxY; y++) {
    for (int x = minX + 1; x <= maxX; x++) {
      if (mask[y * resized.width + x]) {
        final p = resized.getPixel(x, y); int qc = ((p.r.toInt() ~/ 32) << 16) | ((p.g.toInt() ~/ 32) << 8) | (p.b.toInt() ~/ 32);
        if (mask[(y - 1) * resized.width + x]) {
          final pt = resized.getPixel(x, y - 1);
          if (qc == (((pt.r.toInt() ~/ 32) << 16) | ((pt.g.toInt() ~/ 32) << 8) | (pt.b.toInt() ~/ 32))) clustered++;
        }
      }
    }
  }

  int limbPix = 0; final iX = ((maxX - minX + 1) * 0.2).toInt(), iY = ((maxY - minY + 1) * 0.2).toInt();
  for (int y = minY; y <= maxY; y++) {
    for (int x = minX; x <= maxX; x++) if (mask[y * resized.width + x]) {
      if (x < minX + iX || x > maxX - iX || y < minY + iY || y > maxY - iY) limbPix++;
    }
  }
  ...
  for (int y = minY; y <= maxY; y++) {
    for (int x = minX; x <= maxX; x++) if (mask[y * resized.width + x]) {
      final p = resized.getPixel(x, y); uniqueColors.add(((p.r.toInt() ~/ 16) << 16) | ((p.g.toInt() ~/ 16) << 8) | (p.b.toInt() ~/ 16));
    }
  }
  ...
  return {
    ...
    'cornerDensity': 0.0, 'diagonalDensity': 0.0, 'lowerQuadrantSymmetry': 0.0,
    ...
    'hueComplexity': 0.0, 'compactness': (perim * perim) / objectPixelCount, 'limbDensity': limbPix / objectPixelCount,
    'directionalEdge_bias': 0.0,
...
(double, double) _calculateSymmetry(img.Image image, List<bool> mask, int minX, int maxX, int minY, int maxY) {
  ...
  for (int y = minY; y <= maxY; y++) {
    for (int x = minX; x <= (minX + maxX) ~/ 2; x++) {
      final x2 = maxX - (x - minX); if (x2 < minX || x2 > maxX) continue;
      final p1 = image.getPixel(x, y), p2 = image.getPixel(x2, y); hTotal++;
      if (((p1.r - p2.r).abs() + (p1.g - p2.g).abs() + (p1.b - p2.b).abs()) < 100) hMatches++;
    }
  }
  for (int x = minX; x <= maxX; x++) {
    for (int y = minY; y <= (minY + maxY) ~/ 2; y++) {
      final y2 = maxY - (y - minY); if (y2 < minY || y2 > maxY) continue;
      final p1 = image.getPixel(x, y), p2 = image.getPixel(x, y2); vTotal++;
...
double _calculateEdgeDensity(img.Image image, List<bool> mask) {
  ...
  for (int y = 1; y < image.height - 1; y++) {
    for (int x = 1; x < image.width - 1; x++) {
      ...
      final p = image.getPixel(x, y), pR = image.getPixel(x + 1, y), pD = image.getPixel(x, y + 1);
      final lum = (p.r + p.g + p.b) / 3.0, lR = (pR.r + pR.g + pR.b) / 3.0, lD = (pD.r + pD.g + pD.b) / 3.0;
