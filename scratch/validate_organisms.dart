  ...
  for (int i = 0; i < data.length; i++) {
    ...
    try {
      ...
      if (item['types'] is List) {
        ...
        typeList = (item['types'] as String)
            .split(',')
            .map((e) => e.trim())
            .toList();
      }
      if (typeList.isEmpty && item['category'] is String) {
        typeList = (item['category'] as String)
            .split(',')
            .map((e) => e.trim())
            .toList();
      }
      ...
      final animalClass =
          (item['class'] ?? item['animal_class'] as String? ?? 'unknown');
      // final diet = (item['diet'] as String? ?? 'unknown');

      // Check if animalClass is actually a String
      if (animalClass is! String) {
        throw 'animalClass is not a String: ${animalClass.runtimeType}';
      }
    } catch (e) {
